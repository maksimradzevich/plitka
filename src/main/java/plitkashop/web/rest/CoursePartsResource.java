...
import plitkashop.repository.CoursePartsRepository;
...
/**
 * REST controller for managing {@link plitkashop.domain.CourseParts}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CoursePartsResource {
    ...
    public ResponseEntity<CourseParts> updateCourseParts(@RequestBody CourseParts courseParts) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, courseParts.getId().toString()))
            .body(result);
    }
    ...
    public List<CourseParts> getAllCourseParts() {
        log.debug("REST request to get all CourseParts");
        return coursePartsRepository.findAll();
    }
    ...
    public ResponseEntity<CourseParts> getCourseParts(@PathVariable Long id) {
        ...
        Optional<CourseParts> courseParts = coursePartsRepository.findById(id);
